...
export class OrderController {
    ...
    public async getOrdersByCreationDate(req: Request, res: Response): Promise<Response> {
        ...
        try {
            const orders = await this.orderService.getOrdersByCreationDate(new Date(startDate as string), new Date(endDate as string));
            return res.status(200).json(orders);
        } catch (error) {
            return res.status(500).json({ message: 'Internal server error' });
        }
    ...
    public async getOrdersByUpdateDate(req: Request, res: Response): Promise<Response> {
        ...
        try {
            const orders = await this.orderService.getOrdersByUpdateDate(new Date(startDate as string), new Date(endDate as string));
            return res.status(200).json(orders);
        } catch (error) {
            return res.status(500).json({ message: 'Internal server error' });
        }
